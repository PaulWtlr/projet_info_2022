...
class MainWindow:
    ...
    def __init__(self, master):
        ...
        self.btn2users = tk.Button(
            self.frmButton, text='Mode de jeu 2 joueurs', width=25, command=self.mode_2_users)
        self.btn2users.pack(side=tk.RIGHT)

        #Bouton de choix de la stratégie de l'ordinateur#
        self.btnGreedyBot = tk.Button(
            self.frmButton, text='Stratégie Bonne Pioche', width=25, command=self.BonnePiocheBot)
        self.btnGreedyBot.pack(side=tk.RIGHT)

        self.btnAntiGagnantBot = tk.Button(
            self.frmButton, text='Stratégie Anti Gagnant', width=25, command=self.AntiGagnantBot)
        self.btnAntiGagnantBot.pack(side=tk.RIGHT)

        self.btnDBCBot = tk.Button(
            self.frmButton, text='Stratégie DBC', width=25, command=self.DBCBot)
        self.btnDBCBot.pack(side=tk.RIGHT)

        self.btnrandomBot = tk.Button(
            self.frmButton, text='Placement aléatoire (Par défaut)', width=25, command=self.randomBot)
        self.btnrandomBot.pack(side=tk.RIGHT)

        #Bouton de reset du jeu#
        self.btnClear = tk.Button(
            self.frmButton, text='Rejouer', width=25, command=self.onClickClear)
        self.btnClear.pack(side=tk.RIGHT)

        #Affichage des scores du joueurs et du bot#
        self.score_user = 0
        self.score_user_variable = tk.StringVar(
            self.master, f'Score Joueur: {self.score_user}')
        self.score_user_lbl = tk.Label(
            self.master, textvariable=self.score_user_variable)
        self.score_user_lbl.pack()

        self.score_bot = 0
        self.score_bot_variable = tk.StringVar(
            self.master, f'Score Bot: {self.score_bot}')
        self.score_bot_lbl = tk.Label(
            self.master, textvariable=self.score_bot_variable)
    ...
    def DBC_place(self):
        ...
        play_list = [(x + random.choice((-1, 1))*r.random()*25, y +
                      random.choice((-1, 1))*r.random()*25) for (x, y) in DBC_list]
    ...
    def AntiGagnant_place(self):
        if self.count == 0:
            ...
            centre_pol_i = (sum([p[0] for p in list_coords[i]])/len(list_coords[i]),
                            sum([p[1] for p in list_coords[i]])/len(list_coords[i]))
    ...
    def BonnePioche_place(self):
        ...
        self.w.create_oval(xy[0][0]-self.RADIUS, xy[0][1]-self.RADIUS, xy[0]
                           [0]+self.RADIUS, xy[0][1]+self.RADIUS, fill="yellow", tags='train')
        vp = self.get_vp()
        maxi = vp.bot.score
        (x_play, y_play) = (0, 0)
        self.w.delete("train")
        ...
        for (x, y) in xy:
            self.w.create_oval(x-self.RADIUS, y-self.RADIUS, x +
                               self.RADIUS, y+self.RADIUS, fill="yellow", tags='train')
            vp = self.get_vp()
    ...
    def onDoubleClick(self, event):
            ...
            if self.count == 5:
                ...
                if not self.LOCK_FLAG:
                    self.w.create_oval(event.x-self.RADIUS, event.y-self.RADIUS,
                                       event.x+self.RADIUS, event.y+self.RADIUS, fill="red")
        ...
        # Mode de jeu 2 joueurs
            ...
            if self.count % 2 == 1 and not self.LOCK_FLAG:
                self.w.create_oval(event.x-self.RADIUS, event.y-self.RADIUS,
                                   event.x+self.RADIUS, event.y+self.RADIUS, fill="blue")
    ...
    def check_winner(self):
        ...
        if self.game_mode == 0:
            if self.score_user > self.score_bot:
                self.w.create_text(
                    250, 300, text="Le joueur a gagné", fill="red", font=('Helvetica 15 bold'))
                self.w.pack()

            elif self.score_user < self.score_bot:
                self.w.create_text(
                    250, 300, text="L'ordinateur a gagné'", fill="blue", font=('Helvetica 15 bold'))
                self.w.pack()

            else:
                self.w.create_text(250, 300, text="Egalité'",
        ...
        if self.game_mode == 1:
            if self.score_user > self.score_bot:
                self.w.create_text(
                    250, 300, text="Le Joueur rouge a gagné", fill="red", font=('Helvetica 15 bold'))
                self.w.pack()

            elif self.score_user < self.score_bot:
                self.w.create_text(
                    250, 300, text="Le Joueur bleu a gagné", fill="blue", font=('Helvetica 15 bold'))
                self.w.pack()
